...
# match species names
# also match gene names
# insert an extra column for the following porprotion calculation

def name_match(input_partial_fname, input_full_fname, input_gene_fname, output_fname):
    input_partial_df = pd.read_csv(input_partial_fname , header=None, index_col = False) # open pos_neg csv file
    input_full_df = pd.read_csv(input_full_fname , header=None, index_col = False) # open file with complete (full) genome names
    input_gene_df = pd.read_csv(input_gene_fname, header = 0, index_col = 0) # open query_gene.csv
